...
def optimalZeropad(x, fs, f):
    ...
    M = x.size
    print(M)
    period = math.ceil(float(fs)/float(f))
    original = float(M * f) / float(fs)
    print(original)
    padded = int(math.ceil(original))
    print(padded)
    gap = int((padded-original) * period)
    print(gap)
    N = M + gap
    print(N)
    for i in range(gap):
        ...
        print(x.size)
    X = fft(x)
    return 20.0 * np.log10(abs(X[:(N/2)+1]))
